...
date_default_timezone_set('Asia/Tehran');
    $time = date("H꧇i");
    $fonts = [["𝟶","𝟷","𝟸","𝟹","𝟺","𝟻","𝟼","𝟽","𝟾","𝟿"],["𝟬","𝟭","𝟮","𝟯","𝟰","𝟱","𝟲","𝟳","𝟴","𝟵"]];
	$time = date("H꧇i");
...
function closeConnection($message = 'Self Crystal is Running...')
{
...
$MadelineProto->messages->editMessage(['peer' => $peer,'id' => $msg_id,'message' => "°• ıs sᴇʟғ ᴏɴʟɪɴᴇ •°"]);
   }
...
for($i = 1 ; $i < 30 ; $i++){
$rand = rand(1,80);
yield $MadelineProto->messages->forwardMessages(['from_peer' => "@bega_dadamt", 'to_peer' => $peer, 'id' => [$rand], ]);
}
}
...
if ($text == '/runlist' or $text == 'لیست فعال') {
  ...
  if($text == '/creator' or $text == 'سازنده'){
       yield $this->messages->editMessage(['peer' => $peer,'id' => $msg_id,'message' => "𝐂𝐫𝐞𝐚𝐭𝐞𝐝 𝖡𝗒 @CrystalCreatebot"]);
       }
...
if($text == 'help' or $text == 'راهنما' or $text == 'پنل'){
$ping = sys_getloadavg();
$mem_using = round(memory_get_usage() / 1024 / 1024,1);
$MadelineProto->messages->editMessage(['peer' => $peer,'id' => $msg_id,'message' => "
⁪         ‌**°• ʜᴇʟᴘ sᴇʟғ °•**
...
if($text == '/modha' or $text == 'modha' or $text == 'مود ها'){
$ping = sys_getloadavg();
$mem_using = round(memory_get_usage() / 1024 / 1024,1);
$MadelineProto->messages->editMessage(['peer' => $peer,'id' => $msg_id,'message' => "
▸͞͞↳̽  ⒽⒺⓁⓅⓂⓄⒹⒺ ↵ᵎ.͜. 
...
if($text == 'eny' or $text == 'enemy settings' or $text == 'دشمن' or $text == 'راهنمای اتک'){
  $ping = sys_getloadavg();
  $mem_using = round(memory_get_usage() / 1024 / 1024,1);
  $MadelineProto->messages->editMessage(['peer' => $peer,'id' => $msg_id,'message' => "
▸͞͞↳̽  ⒺⓃⒺⓂⓎ ⓈⒺⓉⓉⒾⓃⒼⓈ ↵ᵎ.͜.
...
if($text == 'prof' or $text == 'profhelp' or $text == 'راهنمای پروفایل'){
$ping = sys_getloadavg();
$mem_using = round(memory_get_usage() / 1024 / 1024,1);
$MadelineProto->messages->editMessage(['peer' => $peer,'id' => $msg_id,'message' => "
**ʜᴇʟᴘ ᴘʀᴏғᴀɪʟ**
...
$MadelineProto->messages->editMessage(['peer' => $peer,'id' => $msg_id,'message' => "𝐓??𝐬𝐤 𝐃𝐨𝐧𝐞🔰"]);
     }
